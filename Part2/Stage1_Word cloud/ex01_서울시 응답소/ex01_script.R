...
setwd("D:/workspace/TIL/R_Data_Analysis/Part2/Stage1_Word cloud/ex01_서울시 응답소/")
...
data1 <- readLines("seoul_new.txt") ;data1          #파일에서 읽은 Raw Data(한글 문장)
data2 <- sapply(data1, extractNoun, USE.NAMES=F) ;data2        # 명사들만 있는 데이터(List 형태)
data3 <- head(unlist(data2), 30)
data3 <- unlist(data2) ;data3         # 리스트 형태가 아닌 명사 데이터
data3 <- gsub("\\d+","",data3) 
...
data4 <- read.table("seoul_3.txt") ;data4     # 불필요한 단어가 제거된 명사 데이터
wordcount <- table(data4) ;wordcount
head(sort(wordcount, decreasing=T), 20)
...
v1 <- ("봄이 지나고 여름이고 여름이 지나면 가을입니다.그리고 겨울이죠") ;v1
extractNoun(v1)
v2 <- ("봄이 지나 면여름이고 여름이지나면가을 입니다") ;v2
extractNoun(v2)
